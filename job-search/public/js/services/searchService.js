...
angular.module('searchService', []).factory('searchService', ['$http', function ($http) {
    ...
    //'http://www.authenticjobs.com/api/?api_key=94b7acb384894b18cdca339bab11b574&method=aj.jobs.search&format=json&keywords=php,mysql&perpage=1&callback=JSON_CALLBACK';
    ...
    return {
        getSearchResults: function (callback, searchText) {
            ...
            var tempUrl = baseUrl + searchMethod + '&telecommuting=1&format=json' + searchParameter + '&perpage=5&callback=JSON_CALLBACK';
            return $http.jsonp(tempUrl).success(function(data) {
                callback(data.listings.listing);
            });
        },
        ...
        //        var tempUrl = baseUrl + searchMethod + '&telecommuting=1&format=json' + searchParameter + '&perpage=5&callback=JSON_CALLBACK';
        //        return $http.jsonp(tempUrl).then(basicSuccess);
        //},
